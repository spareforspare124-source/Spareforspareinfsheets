    ...
    <section id="signup" className="section-dark">
      <div className="max-w-[1280px] mx-auto px-6 py-24 lg:py-28 grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <div>
          ...
          <p className="mt-5 max-w-[520px] text-[15px] text-slate-500 leading-relaxed">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm" data-testid="auth-panel">
          <div className="grid grid-cols-2 gap-2 mb-5">
            <button onClick={() => setTab('signup')} data-testid="tab-signup" className={`py-2.5 rounded-lg text-[14px] font-medium transition-colors ${tab === 'signup' ? 'bg-blue-600 text-white' : 'bg-transparent text-slate-500 hover:text-slate-900 border border-slate-200'}`}>Sign Up</button>
            <button onClick={() => setTab('login')} data-testid="tab-login" className={`py-2.5 rounded-lg text-[14px] font-medium transition-colors ${tab === 'login' ? 'bg-blue-600 text-white' : 'bg-transparent text-slate-500 hover:text-slate-900 border border-slate-200'}`}>Log In</button>
          </div>
          {tab === 'signup' ? (
