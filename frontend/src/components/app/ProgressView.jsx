...
// PredictedScoreMini
//
// Track-aware summary card replacing the old "Best score" tile.
...
function PredictedScoreMini({ predictedBySubject, visibleSubjects, examTrack }) {
  const track = (examTrack || '').toUpperCase();
  // Only count subjects that actually have data.
  ...
  if (scored.length === 0) {
    return (
      <div className="rounded-xl border border-[color:var(--color-border)] bg-white p-4">
        <div className="eyebrow-muted">Predicted score</div>
  ...
  if (track === 'IGCSE' || track === 'ASA') {
    ...
    const entries = IGCSE_GRADE_ORDER
      .filter((g) => counts[g])
  ...
  // IB → 1-7 grade counts (e.g. "6 7s, 3 6s")
  if (track === 'IB') {
    const counts = {};
    scored.forEach((p) => {
